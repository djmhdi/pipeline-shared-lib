#!/usr/bin/groovy
package fr.pe.jenkins.plugins.restart

import java.util.logging.Logger

class Restart {

	static Logger logger = Logger.getLogger('fr.pe.jenkins.plugins.restart.Restart')
	...
	static def restart(script, def targetDescriptor, def dictionary) {
		...
		if (serverValidForRestart.contains(type)){
			...
			def starter = script.getClass().classLoader.loadClass( "fr.pe.jenkins.plugins.restart.command.$restartCommand", true, false )?.newInstance()
