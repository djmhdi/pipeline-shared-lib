...
def createRelease(def config) {
	...
	echo "${mvnNextDevVersion}"
		...
		withCredentials([usernamePassword(credentialsId: config.GIT_CREDENTIAL_ID, passwordVariable: 'password', usernameVariable: 'username')]) {
			...
			String mvnDeployFile = "mvn -gs $MAVEN_SETTINGS deploy:deploy-file -Durl=${env[config.repository.envParameterName]} -DrepositoryId=${env[config.repository.credentialId]} -DpomFile=${config.bundlePom} -Dfile=${config.bundleRelativePath} -Dpackaging=${config.bundle.packaging}"
			echo "${mvnDeployFile}"
			sh "${mvnDeployFile}"
		}
